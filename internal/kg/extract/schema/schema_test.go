...
func TestIndexOnClauseMaySpanLines(t *testing.T) {
	...
	if len(idx) != 85 {
		t.Errorf("extracted %d indexes, want 85", len(idx))
	}
...
func TestCorpusCensus(t *testing.T) {
	...
	want := map[string]int{
		kindTable: 40, kindColumn: 374, kindIndex: 85, kindConstraint: 128, kindTrigger: 8,
	}
...
func TestMultiLineAlterTableIsExtracted(t *testing.T) {
	...
	// Thirty-four tables carry the discriminator (E2.1 added telemetry_sample;
	...
	// on_call_schedule and on_call_participant — all declared inline in their
	// CREATE TABLE, not a multi-line ALTER TABLE, but counted the same way:
	// this assertion sweeps every tenant_id column node regardless of which
	...
	if scoped != 34 {
		t.Errorf("%d tables carry tenant_id, want 34", scoped)
	}
