...
func TestEveryTableIsANode(t *testing.T) {
	...
	want := []string{
		...
		"configuration_object", "dependency_edge", "dependency_suppression", "escalation_policy", "escalation_tier",
...
func TestIndexOnClauseMaySpanLines(t *testing.T) {
	...
	if len(idx) != 88 {
		t.Errorf("extracted %d indexes, want 88", len(idx))
	}
...
func TestCorpusCensus(t *testing.T) {
	...
	want := map[string]int{
		kindTable: 42, kindColumn: 390, kindIndex: 88, kindConstraint: 135, kindTrigger: 8,
	}
...
func TestMultiLineAlterTableIsExtracted(t *testing.T) {
	...
	// Thirty-six tables carry the discriminator (E2.1 added telemetry_sample;
	...
	// and escalation_tier — all declared inline in their CREATE TABLE, not a
	...
	if scoped != 36 {
		t.Errorf("%d tables carry tenant_id, want 36", scoped)
	}
