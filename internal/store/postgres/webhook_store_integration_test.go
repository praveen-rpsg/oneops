...
func TestWebhookStore_Integration(t *testing.T) {
	...
	if err := s.MarkResult(ctx, "d1", time.Time{}, events.StatusDelivered, 0, 200, now, time.Time{}); err != nil {
	...
	if err := s.MarkResult(ctx, "d2", time.Time{}, events.StatusDeadLetter, 3, 500, now, time.Time{}); err != nil {
	...
	if err := s.MarkResult(ctx, "d1", time.Time{}, events.StatusDelivered, 0, 200, now.Add(-72*time.Hour), time.Time{}); err != nil {
