...
func TestEvaluator_FlapSuppression_OscillationCollapsesToOneTransition(t *testing.T) {
	...
	e := NewEvaluator(store, tel, notifier, nil, newFakeMaintenanceChecker(), newFakeDependencyChecker(), NopMetrics{}, quiet(), Config{Concurrency: 1})
	e.now = clock.now
...
func TestEvaluator_FlapSuppression_PendingSurvivesNewEvaluatorInstance(t *testing.T) {
	...
	e1 := NewEvaluator(store, tel, notifier1, nil, newFakeMaintenanceChecker(), newFakeDependencyChecker(), NopMetrics{}, quiet(), Config{Concurrency: 1})
	e1.now = clock.now
	...
	e2 := NewEvaluator(store, tel, notifier2, nil, newFakeMaintenanceChecker(), newFakeDependencyChecker(), NopMetrics{}, quiet(), Config{Concurrency: 1})
	e2.now = clock.now
	setMetric(tel, rule, clock.t, true)
