...
- [~] E7.1 **NOC operational-overview projection.** One `GET /v1/admin/noc/overview` endpoint, one RLS-scoped appPool read (RLS does tenant isolation — no explicit predicate, no privileged pool), `PermAdmin`, transient DTO, bounded/indexed COUNT+small-list queries. Aggregates the whole loop's CURRENT state: incidents (open total, by_status, by_severity, root-grouping summary via `incident.root_incident_id`), firing alerts (count + by severity — new `CountFiring`-style store query, not list-all-in-memory), asset health (reuse `AssetRepository.Health()`), on-call now (list active schedules + `OnCall(now)` each), active escalations (COUNT `escalation_state` status='active' — table exists since E5.2b-2). Telemetry summary DEFERRED to keep it bounded. Additive openapi (contract bijection). No new tables/entities. Fast-CTO-self-review tier (read-only projection) but verify tenant scoping + query bounds. ▶ CURRENT
- [ ] E7.2 **Live NOC screen** — a minimal self-contained page (served embedded, no React-build fight with `webdist/`) that polls `/admin/noc/overview` and renders operational-state cards, auto-refresh. This is where "SEE it" lands. A read-only VIEW/projection, not a reified entity. Carries frontend + auth-in-browser concerns → its own story, right after E7.1.
