...
- [~] E3.3a Maintenance windows / blackout: operator-declared time window during which a target's firing→incident/notify is suppressed (planned maintenance shouldn't page). Tenant-owned `maintenance_window`; evaluator checks for an active window over the affected CI before committing a firing; suppression is recorded (audit/annotation), never silently lost. Split out of the old bundled E3.3 — lower-risk, universally-needed, ships first.  ▶ CURRENT
- [ ] E3.3b Dependency-aware suppression: when a root CI is down, suppress firings on its downstream/dependent CIs (they're down *because* the root is) — uses the CMDB graph (E1 relationships). The harder half: graph traversal + cycle handling + a sound "root is actually down" signal + not swallowing a genuinely independent downstream failure. Own story, own review.
