...
- [ ] E3.4 **Alert-rule symptom-class taxonomy** — **FOUNDER-APPROVED 2026-08-05 ("yes, plan it").** Add a symptom-class to `alert_rule` (e.g. `availability`/`reachability` vs `resource` vs `other`) so suppression + correlation can act on WHAT a rule detects, not just its severity. This is the primitive that upgrades E3.3b dependency suppression from all-or-nothing (its documented Honest Bound) to reachability-scoped, and makes E4.2 root-cause grouping precise. **Design decisions to make first (challenge-before-build):** (a) the class enumeration; (b) how a rule is classified — explicit operator-set field vs inferred from metric name vs hybrid with an operator override; (c) backfill of existing rules to a safe default; (d) whether class is required or optional on create. Foundational + modeling-heavy → its own ADR + careful review. NOT reified as a new entity — a column on `alert_rule`. Sequence: buildable independently; E4.2 can start heuristically without it, but the class-scoped refinements of E3.3b (ADR-ALERTING-003 future work) and E4.2 root-cause ranking both consume it — do E3.4 before those refinements.
...
- [ ] E4.2 Noise reduction, grouping, root-cause candidate suggestion (heuristic first; ML later in E13); topology-window grouping via the CMDB graph  ▶ CURRENT
