...
- E5.2 On-call, scheduling & paging — **SPLIT (CTO, 2026-08-05) into E5.2a + E5.2b** (founder sequence: E3.4 → E5.2):
  ...
  - [ ] E5.2b **Escalation policies & paging.** `escalation_policy` + ordered `escalation_tier` (schedule to page + wait-before-next). Leader-gated escalation worker mirroring `notification.Worker` (ClaimDue → still-unacked? → page the tier's on-call user → advance; ack/resolve cancels). Pages by enqueuing an **email** Notification to the on-call user (no notification-worker change — recipient=email). Trigger **decoupled**: a reconciler seeds escalations for open alert-incidents (alerting hot path untouched). **Deferred:** per-asset/service policy routing (MVP = tenant-default policy), SMS/push channels. Depends on E5.2a (needs who's-on-call resolution). **HARD REQUIREMENT from the E5.2a review (nit 1):** E5.2a re-verifies tenant membership only at participant ADD time, so a revoked member still resolves as on-call. At PAGE time E5.2b MUST re-check the resolved on-call user is still an ACTIVE tenant member before notifying — skip/advance to the next tier (or fall through per policy) if not. Do not page a revoked user.  ▶ CURRENT
