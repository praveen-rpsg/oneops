...
- E5.2 On-call, scheduling & paging — **SPLIT (CTO, 2026-08-05) into E5.2a + E5.2b** (founder sequence: E3.4 → E5.2):
  ...
  - E5.2b **Escalation policies & paging** — **SPLIT (CTO, 2026-08-05) into config + engine** (config is low-risk additive CRUD; the engine is a concurrency-heavy state machine deserving full independent review):
    - [~] E5.2b-1 **Escalation policy + tier config (CRUD).** Tenant-owned `escalation_policy` (name, status; MVP = one tenant-default) + ordered `escalation_tier` (position, `on_call_schedule_id` FK = which schedule to page, `wait_seconds` before advancing). CRUD + tier reorder (reuse E5.2a's deferred-position-unique pattern). FK to `on_call_schedule` re-verified same-tenant. NO engine/worker/paging. Low-risk additive → fast CTO self-review tier.  ▶ CURRENT
    - [ ] E5.2b-2 **Escalation engine (seed → page → escalate → cancel).** `escalation_state` work-queue (one active per incident: `incident_id` unique, `current_tier_index`, `next_attempt_at`, `claimed_at` fence, status active|acked|resolved|exhausted). A leader-gated **seeding reconciler** enrolls open, unacked, alert-sourced incidents (alerting hot path untouched). A leader-gated **escalation worker** mirroring `notification.Worker` (ClaimDue → incident still open+unacked? if acked/resolved mark terminal & stop → resolve who's-on-call for the tier's schedule at now → **PAGE-TIME ACTIVE-MEMBERSHIP RE-CHECK (E5.2a review nit 1): do NOT page a revoked user — skip/advance** → enqueue an **email** Notification to the on-call user (no notification-worker change, recipient=email) → advance tier or mark exhausted). Idempotent + at-least-once + fenced on `claimed_at` (no double-page). **Deferred:** per-asset/service policy routing (MVP tenant-default), SMS/push. Full-review tier. Depends on E5.2a + E5.2b-1.
