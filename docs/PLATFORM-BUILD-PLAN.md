...
- [ ] E5.3 SLA/SLO tracking on work items (business hours, pause-on-hold, breach escalation)  ← deferred again; founder chose CONSOLE OPERATIONAL ACTIONS next (2026-08-06, see E-ACT)
...
### E-ACT — Console operational actions  `[~]` — **FOUNDER CHOSE THIS NEXT (2026-08-06).** Turn the read-only 7-screen Cloudscape console into one operators WORK in: wire WRITE actions to the EXISTING backend endpoints. New cross-cutting UI concerns (every action honors these): **optimistic-locking `row_version` round-trip** (read carries it, the mutation sends it back, 409 → refetch+retry, surfaced clearly), RFC 7807 error surfacing (422 validation, 409 conflict, 403), **confirmation modals for destructive/irreversible actions**, form validation before submit, and a post-action refetch so the view reflects the new state. Sequenced by daily-operational value.
  ...
  - [ ] E-ACT.2 **Alert-rule CRUD** — create/edit/enable-disable/delete alert rules from the alerts board (`POST/PATCH/DELETE /v1/admin/alert-rules`), incl. severity + symptom_class + threshold/comparator/dwell; also add the additive `current_incident_id` DTO field (the E7.3c/ADR-NOC-005 gap) so the linked-incident column can exist — a small Go change (full-review) bundled here or split.  ▶ CURRENT
  - [ ] E-ACT.3 **Maintenance-window create/cancel** from the alerts/asset context (`POST/DELETE /v1/admin/maintenance-windows`).
  - [ ] E-ACT.4 **On-call schedule + roster management** from the on-call board (create schedule, add/remove/reorder participants — `/on-call-schedules` + `/participants`).
  - [ ] E-ACT.5 **Escalation policy + tier management** from a new config surface (`/escalation-policies` + `/tiers`).
