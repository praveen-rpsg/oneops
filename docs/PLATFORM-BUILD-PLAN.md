...
- E9 Dashboards (trends over time) — **SPLIT (CTO, 2026-08-06) data-then-viz.** Scout finding: **NO alert-firing history exists** (only `alert_rule.last_state` — alerts are derived, never logged per-firing); so "alert volume over time" is honestly the **alert-*sourced* incident volume** (`incident.source='alert'` from E4.1) — a true firing log would need a new `alert_event` table (deferred to E3 backlog, NOT E9). Telemetry trends REUSE the existing `/v1/admin/telemetry?resolution=rollup_5m`. Charts = Cloudscape-native `LineChart`/`BarChart`/`AreaChart` (already bundled, no new dep).
  - [~] E9.1 **Incident-trends projection** — `GET /v1/admin/dashboards/incident-trends?from=&to=&bucket=` → opened (and resolved) counts per time bucket, split by severity and by source (manual|alert; the alert-volume proxy). Bucketed via `date_trunc`/`time_bucket` over `incident.created_at`; ADDITIVE index `ix_incident_tenant_created_at (tenant_id, created_at DESC)` (migration). Transient DTO, RLS appPool, PermAdmin, bounded (cap buckets), additive openapi. Two-tenant isolation test that BITES. Honest bound in ADR: alert-volume is the incident-source proxy, not a firing log.  ▶ CURRENT
  - [ ] E9.2 **Dashboards screen (Cloudscape)** — a "Dashboards" nav item + screen: Cloudscape `LineChart`/`BarChart` over E9.1 (incident volume over time by severity/source) + a telemetry-trend chart reusing `/v1/admin/telemetry`. Poll/refresh; dark/light; deep-linkable. Rounds out visibility.
