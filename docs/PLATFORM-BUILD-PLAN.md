...
- [ ] E7.2 **Live NOC screen** — a minimal self-contained page (served embedded, no React-build fight with `webdist/`) that polls `/admin/noc/overview` and renders operational-state cards, auto-refresh. This is where "SEE it" lands. A read-only VIEW/projection, not a reified entity. Carries frontend + auth-in-browser concerns → its own story, right after E7.1. ▶ CURRENT
- [ ] E7.3 Topology map (CMDB graph visual over E1 relationships), health propagation; drill-down per-domain NOC views (incidents/alerts/on-call boards) — overlaps E9.
