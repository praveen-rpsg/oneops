...
- E7.3 Drill-down NOC views + topology — **SPLIT (CTO, 2026-08-06):**
  - [~] E7.3a **NOC incident board** — drill from the overview into the actual open incidents, grouped root→collateral, sorted by severity, click → detail (title/severity/status/assignee) + timeline. Reuse the EXISTING `/v1/admin/incidents` (list w/ status filter + root_incident_id/is_root from E4.2), `/{id}`, `/{id}/timeline` — add a small `/v1/admin/noc/incident-board` projection ONLY if server-side grouping correctness needs it (else client-side group the bounded list). A board VIEW extending the /noc page family; read-only, no new entity. Visual-verify.  ▶ CURRENT
  - [ ] E7.3b **CMDB topology map** — visual of the asset dependency graph (E1 relationships) with incident/health overlay; reuses the graph-traversal projections. Graph-viz frontend (self-contained, no external libs — inline SVG/canvas).
  - [ ] E7.3c Alerts board + on-call board (per-domain drill-downs), shift-handover view.
