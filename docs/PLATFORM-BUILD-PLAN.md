...
- [~] E3.4 **Alert-rule symptom-class taxonomy** — **FOUNDER-APPROVED + DESIGN LOCKED 2026-08-05.** `alert_rule.symptom_class` enum = **`availability | resource | unspecified`** (minimal — the one distinction suppression/correlation need: does the symptom cascade through dependencies? availability does, resource doesn't). **Explicit operator-set field, default `unspecified`** (CTO call on founder's "follow best"): existing rules backfill to `unspecified` and behave EXACTLY as today — no metric-name inference (too fragile / silently mis-suppressing). Optional on create, patchable. **E3.4 = the PRIMITIVE ONLY** — field + create/patch + backfill + validation + tests + ADR + DTO/openapi (additive). **NO behavior change**: the class-scoped refinements of E3.3b dependency suppression and E4.2 root-cause ranking are SEPARATE follow-on stories that consume this primitive, done AFTER E3.4. NOT a reified entity — a column on `alert_rule`. Non-breaking, independently reviewable.  ▶ CURRENT
...
- [ ] E5.2 On-call, scheduling & paging (rotations, escalation policies, notify via E-notification)  ← next after E3.4 (founder sequence 2026-08-05: E3.4 → E5.2)
