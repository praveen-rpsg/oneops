...
- E5.2 On-call, scheduling & paging — **SPLIT (CTO, 2026-08-05) into E5.2a + E5.2b** (founder sequence: E3.4 → E5.2):
  - [~] E5.2a **On-call schedules & rotations (who's on call now).** Tenant-owned `on_call_schedule` (name, `handoff_interval_seconds`, `rotation_start_at`, status) + `on_call_participant` (ordered users; FK `app_user` re-verified via ACTIVE membership per ADR-IDENTITY-002, like incident-assignee). Who's-on-call(T) = **deterministic computed rotation** `index = floor((T − rotation_start)/interval) mod participants` — NO JSON rules engine, NO per-shift rows (that's a framework). CRUD + reorder + a "who's on call now" endpoint. Reduced-concept safe (legitimate operational objects, not false-nouns). **Deferred (honest bound):** overrides/coverage-swaps, multi-layer follow-the-sun. Self-contained — no escalation dependency.  ▶ CURRENT
  - [ ] E5.2b **Escalation policies & paging.** `escalation_policy` + ordered `escalation_tier` (schedule to page + wait-before-next). Leader-gated escalation worker mirroring `notification.Worker` (ClaimDue → still-unacked? → page the tier's on-call user → advance; ack/resolve cancels). Pages by enqueuing an **email** Notification to the on-call user (no notification-worker change — recipient=email). Trigger **decoupled**: a reconciler seeds escalations for open alert-incidents (alerting hot path untouched). **Deferred:** per-asset/service policy routing (MVP = tenant-default policy), SMS/push channels. Depends on E5.2a (needs who's-on-call resolution).
